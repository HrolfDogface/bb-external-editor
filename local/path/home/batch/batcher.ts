export async function main(ns: NS) {
  ...
  while (true){
    ...
    const growDelay: number = weakenTime - growTime + 2;
    const weaken2Delay: number = 4;
    const batchDelay: number = 8;
    ...
    while (true)
    {
      ...
      hackPids.push(ns.exec("batch/H_worker.js", exHost, hackThreads, targetHost, hackTime, Date.now() + hackTime + hackDelay + 2));
      ns.exec("batch/W_worker.js", exHost, weaken1Threads, targetHost, weakenTime, Date.now() + weakenTime + weaken1Delay + 2);
      ns.exec("batch/G_worker.js", exHost, growthThreads, targetHost, growTime, Date.now() + growTime + growDelay + 2);    
      ns.exec("batch/W_worker2.js", exHost, weaken2Threads, targetHost, weakenTime, Date.now() + weakenTime + weaken2Delay + 2, ns.pid, workerPid);
